...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    ##### FINISH TODOS HERE #####
     #create a V* table and initialize it with 0

    V=np.zeros(num_spaces,dtype=float)
    #get reward and transition p
    R, P = get_rewards_and_transitions_from_env(env)

    # first part find V*
    for _ in range(max_iterations):
        delta=0
        for s0 in range(num_spaces):
            maxv=-1
            maxa=0
            for a in range(num_actions):    
                value=0
                for s1 in range(num_spaces):
                    value+=P[s0,a,s1]*(R[s0,a,s1]+gamma*V[s1])
                if maxv==-1:
                  maxv=value
                  maxa=a
                elif value>maxv:
                  maxv=value
                  maxa=a
            delta=max(delta,np.abs(maxv-V[s0]))
            V[s0]=maxv
        if delta<eps:
            break
    #second part find policy*
    for s0 in range(num_spaces):
        maxv=-1
        maxa=0
        for a in range(num_actions):
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    R, P = get_rewards_and_transitions_from_env(env)
    for _ in range(max_iterations):
        newpolicy=np.zeros(num_spaces,dtype=int)
        Vpi=np.zeros(num_spaces,dtype=float)
        #policy evaluation
        while True:
            delta=0
            for s0 in range(num_spaces):
                V=0
                for s1 in range(num_spaces):
                    #policy[s0]==the action we are going to do
                    a=policy[s0]
                    # value+=P[s0,a,s1]*(R[s0,a,s1]+gamma*V[s0,0])
                    V+=P[s0,a,s1]*(R[s0,a,s1]+ gamma*Vpi[s1] )
                delta=max(delta,np.abs(V-Vpi[s0]))
                Vpi[s0]=V
            if delta<eps:
                break
        #stable policy
        for s0 in range(num_spaces):
            ...
            for a in range(num_actions):
                value=0
                for s1 in range(num_spaces):
                        value+=P[s0,a,s1]*(R[s0,a,s1]+gamma*Vpi[s1])
                if maxv==-1:
                    maxv=value
                    maxa=a
                elif value>maxv:
